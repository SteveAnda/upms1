package com.anda.shiro.filter;

import com.alibaba.fastjson.JSONObject;
import com.anda.base.BaseConstants;
import com.anda.shiro.session.UpmsSessionDao;
...
import util.RequestParameterUtil;
...
public class UpmsAuthenticationFilter extends AuthenticationFilter{
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("upms-service-client").get("upms.type");
        session.setAttribute(BaseConstants.UPMS_TYPE, upmsType);
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer ssoServerUrl = new StringBuffer(PropertiesFileUtil.getInstance("upms-service-client").get("upms.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("upms-service-client").get("upms.type");
        if ("server".equals(upmsType)) {
        ...
        // ssoServerUrl.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("upms-service-client").get("upms.appID"));
        // 回跳地址
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        //todo: 存到redis数据库中会自动序列化？然后get的时候需要+Serializable类型的对象？
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        ...
                        LOGGER.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(UPMS_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
